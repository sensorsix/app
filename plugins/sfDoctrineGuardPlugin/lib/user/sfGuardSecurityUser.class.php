...
class sfGuardSecurityUser extends sfBasicSecurityUser
{
   ...
   * @return bool
   */
  public function hasRoadmapAccess()
  {
    return $this->hasCredential('admin') || $this->getGuardUser()->is_super_admin || in_array($this->getGuardUser()->account_type, array('Light','Pro', 'Enterprise'));
  }

  /**
   * @param $context sfContext
   * @return bool
   */
  public function verifyLightAccess($context) {
    $forbidden = array(
      'analyze'     => array('*'),
      'wall'        => array('*'),
      'criterion'   => array('index', 'new', 'create'),
    );

    if (
      $this->getGuardUser()->account_type == 'Light' &&
      array_key_exists($context->getModuleName(), $forbidden) &&
      (array_search('*', $forbidden[$context->getModuleName()]) !== false || array_search($context->getActionName(), $forbidden[$context->getModuleName()]) !== false)
    ){
      return false;
    }
