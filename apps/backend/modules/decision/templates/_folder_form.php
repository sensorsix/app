...
<td>
  ...
  <?php endif ?>
  <span id="edit-url" data-url="<?php echo url_for('@decision\editFolder?id=' . $form->getObject()->id) ?>"></span>
  <script>
    $(function () {
      ...
      $(".autosave").autosave({
        url     : "<?php echo url_for('@decision\updateFolder?id=' . $form->getObject()->id) ?>",
        method  : "post",
        grouped : true,
